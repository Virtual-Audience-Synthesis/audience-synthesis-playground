...
app.layout = html.Div(
    [
        html.Div(
            ...
            children=[
                html.H3(app.title, style={"color": "#CECECE", "textAlign": "center"}),
                # Audio soundwave
                dcc.Loading(
                    id="soundwave-loading",
                    type="circle",
                    children=html.Div(
                        dcc.Graph(id="soundwave-fig", animate=True),
                        style={"height": "200px"},
                    ),
                ),
                # Audience Panel
                html.Table(
                    [
                        html.Tr(
                            [
                                html.Td(
                                    html.H5("Number of Persons"), style={"width": "15%"}
                                ),
                                html.Td(
                                    dcc.Input(
                                        id="n-person",
                                ...
                                html.Td(
                                    html.H5("Female Ratio (%)"), style={"width": "15%"}
                                ),
                                html.Td(
                                    dcc.Slider(
                                        id="female-slider",
                                ...
                                html.Td(
                                    [
                                        dcc.Loading(
                                            ...
                                            type="circle",
                                            children=html.Div(
                                                dcc.Graph(id="claps-fig", animate=True)
                                            ),
                                        ),
                                        dcc.Slider(
                                ...
                                html.Td(
                                    [
                                        dcc.Loading(
                                            ...
                                            type="circle",
                                            children=html.Div(
                                                dcc.Graph(id="boos-fig", animate=True)
                                            ),
                                        ),
                                        dcc.Slider(
                                            id="booing-intensity",
        ...
        dcc.Interval(id="update", interval=100 * 10, n_intervals=0),
    ],
    style={"fontFamily": "Calibri, sans-serif", "fontSize": "16"},
)
...
def update_audio(
    ...
    n_person: int,
    female_ratio: float,
    clapping_intensity: float,
    whistling_intensity: float,
    laughter_intensity: float,
    booing_intensity: float,
):
    ...
    root_whistle = utils_audio.spawnWhistles(
        ...
        pea_radius=50,
        bump_radius=150,
        fipple_freq_mod=0.35,
        fipple_gain_mod=0.35,
        noise_gain=0.2,
        base_freq=5000,
        sine_rate=4000,
        pole=0.95,
        ...
        load_path=os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "data", "whistle.wav"
        ),
    )
    ...
    laughters = utils_audio.spawnLaughter(
        ...
        female_laughter_path=os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "data", "female_laughter.npy"
        ),
        male_laughter_path=os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "data", "male_laughter.npy"
        ),
        stereo=True,
    ...
    female_boos = utils_audio.spawnBoos(
        n_female_booing, SR, DURATION_IN_SEC, "dashboard/data/female-boos.npy"
    )
    ...
    male_boos = utils_audio.spawnBoos(
        n_male_booing, SR, DURATION_IN_SEC, "dashboard/data/male-boos.npy"
    )
    ...
    # Combine audios
    audio = claps + whistles + laughters_resampled + boos
    #####
...
@app.callback(
    ...
    [Input("audio", "data"), Input("stream-counter", "data")],
)
...
def plot_soundwave(audio: np.ndarray, current_x: int, title: str) -> go.Figure:
    ...
    fig.update_layout(
        ...
        margin=dict(l=10, r=10, b=50, t=0 if title == "Soundwave" else 25, pad=0),
    )
...
if __name__ == "__main__":
    # TODO: debug=False
    app.run_server(host="0.0.0.0", port=os.getenv("PORT", 8500), debug=True)
